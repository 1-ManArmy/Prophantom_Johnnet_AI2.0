#!/usr/bin/env python3
"""
Cv/Resume Optimization And Career Guidance Agent Ollama Engine
Specialized Ollama integration for cv_smash agent
"""
...
class CvSmashOllamaEngine:
    """Specialized Ollama engine for cv_smash agent"""
    ...
    def __init__(self):
        ...
        self.specialization = "resume_optimization"
        
        # Specialized prompts for resume_optimization
        self.system_prompts = {
            'default': f"You are an expert AI agent specializing in {self.specialization}. Your capabilities include: ['resume_analysis', 'skill_assessment', 'career_guidance']",
            'analysis': f"As a {self.specialization} specialist, analyze the following with expertise in ['resume_analysis', 'skill_assessment', 'career_guidance']",
            'creative': f"Using your {self.specialization} expertise, create innovative solutions leveraging ['resume_analysis', 'skill_assessment', 'career_guidance']",
            'technical': f"Provide technical guidance on {self.specialization} using your knowledge of ['resume_analysis', 'skill_assessment', 'career_guidance']"
        }
    
    async def generate_specialized_response(self, prompt: str, context: Dict[str, Any], response_type: str = 'default') -> str:
        """Generate response with resume_optimization specialization"""
        try:
            ...
            Focus on actionable insights and leverage these capabilities: ['resume_analysis', 'skill_assessment', 'career_guidance']
            """
    ...
    async def analyze_with_specialization(self, data: str, analysis_type: str = 'comprehensive') -> Dict[str, Any]:
        """Perform specialized analysis using resume_optimization expertise"""
        try:
            analysis_prompt = f"""
            ...
            ['resume_analysis', 'skill_assessment', 'career_guidance']
            
            Provide analysis in JSON format:
            {{
                "specialization_insights": {"key insights specific to resume_optimization"},
                "recommendations": [{"actionable recommendations"}],
                "confidence_score": 0.95,
                "expertise_level_required": "intermediate",
                "follow_up_suggestions": [{"suggested next steps"}],
                "specialized_metrics": {"resume_optimization_specific_metrics"}
            }}
            """
    ...
    async def generate_creative_solution(self, problem: str, constraints: Dict[str, Any] = None) -> Dict[str, Any]:
        """Generate creative solutions using resume_optimization expertise"""
        try:
            ...
            Your specialized capabilities: ['resume_analysis', 'skill_assessment', 'career_guidance']
    ...
    async def validate_specialized_output(self, output: str, expected_format: str = 'text') -> Dict[str, Any]:
        """Validate output meets resume_optimization standards"""
        try:
            validation_prompt = f"""
