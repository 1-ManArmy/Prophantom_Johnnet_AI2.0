#!/usr/bin/env python3
"""
Pdf Analysis And Document Intelligence Agent Ollama Engine
Specialized Ollama integration for pdf_mind agent
"""
...
class PdfMindOllamaEngine:
    """Specialized Ollama engine for pdf_mind agent"""
    
    def __init__(self):
        self.ollama_service = OllamaService()
        self.primary_model = "deepseek-coder:6.7b"
        self.analysis_model = "phi3:14b"
        self.creative_model = "qwen2.5:7b"
        self.specialization = "document_analysis"
        
        # Specialized prompts for document_analysis
        self.system_prompts = {
            'default': f"You are an expert AI agent specializing in {self.specialization}. Your capabilities include: ['pdf_parsing', 'content_extraction', 'document_summarization']",
            'analysis': f"As a {self.specialization} specialist, analyze the following with expertise in ['pdf_parsing', 'content_extraction', 'document_summarization']",
            'creative': f"Using your {self.specialization} expertise, create innovative solutions leveraging ['pdf_parsing', 'content_extraction', 'document_summarization']",
            'technical': f"Provide technical guidance on {self.specialization} using your knowledge of ['pdf_parsing', 'content_extraction', 'document_summarization']"
        }
    
    async def generate_specialized_response(self, prompt: str, context: Dict[str, Any], response_type: str = 'default') -> str:
        """Generate response with document_analysis specialization"""
        try:
            ...
            Focus on actionable insights and leverage these capabilities: ['pdf_parsing', 'content_extraction', 'document_summarization']
            """
    ...
    async def analyze_with_specialization(self, data: str, analysis_type: str = 'comprehensive') -> Dict[str, Any]:
        """Perform specialized analysis using document_analysis expertise"""
        try:
            analysis_prompt = f"""
            ...
            ['pdf_parsing', 'content_extraction', 'document_summarization']
            
            Provide analysis in JSON format:
            {{
                "specialization_insights": {"key insights specific to document_analysis"},
                "recommendations": [{"actionable recommendations"}],
                "confidence_score": 0.95,
                "expertise_level_required": "intermediate",
                "follow_up_suggestions": [{"suggested next steps"}],
                "specialized_metrics": {"document_analysis_specific_metrics"}
            }}
            """
    ...
    async def generate_creative_solution(self, problem: str, constraints: Dict[str, Any] = None) -> Dict[str, Any]:
        """Generate creative solutions using document_analysis expertise"""
        try:
            ...
            Your specialized capabilities: ['pdf_parsing', 'content_extraction', 'document_summarization']
    ...
    async def validate_specialized_output(self, output: str, expected_format: str = 'text') -> Dict[str, Any]:
        """Validate output meets document_analysis standards"""
        try:
            validation_prompt = f"""
