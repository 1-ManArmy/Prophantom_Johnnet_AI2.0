#!/usr/bin/env python3
"""
Emotional Intelligence And Support Agent Ollama Engine
Specialized Ollama integration for emo_ai agent
"""
...
class EmoAiOllamaEngine:
    """Specialized Ollama engine for emo_ai agent"""
    ...
    def __init__(self):
        ...
        # Specialized prompts for emotional_support
        self.system_prompts = {
            'default': f"You are an expert AI agent specializing in {self.specialization}. Your capabilities include: ['emotion_detection', 'empathy_responses', 'mental_health_support']",
            'analysis': f"As a {self.specialization} specialist, analyze the following with expertise in ['emotion_detection', 'empathy_responses', 'mental_health_support']",
            'creative': f"Using your {self.specialization} expertise, create innovative solutions leveraging ['emotion_detection', 'empathy_responses', 'mental_health_support']",
            'technical': f"Provide technical guidance on {self.specialization} using your knowledge of ['emotion_detection', 'empathy_responses', 'mental_health_support']"
        }
    
    async def generate_specialized_response(self, prompt: str, context: Dict[str, Any], response_type: str = 'default') -> str:
        """Generate response with emotional_support specialization"""
        try:
            ...
            Focus on actionable insights and leverage these capabilities: ['emotion_detection', 'empathy_responses', 'mental_health_support']
            """
    ...
    async def analyze_with_specialization(self, data: str, analysis_type: str = 'comprehensive') -> Dict[str, Any]:
        """Perform specialized analysis using emotional_support expertise"""
        try:
            analysis_prompt = f"""
            ...
            ['emotion_detection', 'empathy_responses', 'mental_health_support']
            
            Provide analysis in JSON format:
            {{
                "specialization_insights": {"key insights specific to emotional_support"},
                "recommendations": [{"actionable recommendations"}],
                "confidence_score": 0.95,
                "expertise_level_required": "intermediate",
                "follow_up_suggestions": [{"suggested next steps"}],
                "specialized_metrics": {"emotional_support_specific_metrics"}
            }}
            """
    ...
    async def generate_creative_solution(self, problem: str, constraints: Dict[str, Any] = None) -> Dict[str, Any]:
        """Generate creative solutions using emotional_support expertise"""
        try:
            ...
            Your specialized capabilities: ['emotion_detection', 'empathy_responses', 'mental_health_support']
    ...
    async def validate_specialized_output(self, output: str, expected_format: str = 'text') -> Dict[str, Any]:
        """Validate output meets emotional_support standards"""
        try:
            validation_prompt = f"""
